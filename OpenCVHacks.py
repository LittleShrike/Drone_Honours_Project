'''
A module that will bundles small functions into a chain of useful ones. Feel free to copy this into your code, just credit appropriately.
...
def ImagePhalanx (images,linewidthbetween=0,bgcol=(0,0,0)):
    '''
    Creates a composite image using a list of image objects, 
    with an optional line space in between and background colour. Horizontal arrangement.
    '''
    if len (images) <= 0:
...
def ImageVertigo (images,linewidthbetween=0,bgcol=(0,0,0)):
    '''
    Creates a composite image using a list of image objects, 
    with an optional line space in between and background colour. Vertical arrangement.
    '''
    if len (images) <= 0:
...
def ImageCatalog (images,imagesPerRow= 0,linewidthbetween=0,bgcol=(0,0,0)):
    '''
    Creates a composite image using a list of image objects, 
    with an optional line space in between and background colour. Grid arrangement.
    '''
    if len (images) <= 0:
...
def NamedImageCatalog (imagesWithNames,imagesPerRow= -1,linewidthbetween=0,bgcol=(0,0,0),text_scale=1.0,textcol = (255,255,255)):
    '''
    Creates a composite image using a list of image objects, 
    with an optional line space in between and background colour. 
    Grid arrangement withe very frame named.
    '''
...
def CenterPointOfContour(theContour):
    '''
    Attempts to find the center point of a given contour, 
    if it fails then it returns (-1,-1)
    '''
    try:
